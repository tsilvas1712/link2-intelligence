...
class ProcessDatasysJob implements ShouldQueue
{
    ...
    public function handle(): void
    {
        ...
        foreach ($this->data as $key => $value) {
            $datasync[$key] = is_array($value) ? null : trim($value);
        }

        try {

            $array = [];
            array_push($array, $datasync);

            $mongo_sync = SyncMongo::create([
                'data' => json_encode($datasync),
                'migrated' => false,
            ]);

            ETLDatasysJob::dispatch($mongo_sync->data, $mongo_sync->id);
